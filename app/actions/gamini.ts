"use server";
import { ImprovementType } from "@/interfaces";
...
export async function generateContent(title: string, category: string = "", tags: string[] = []) {
    try {
...
Start directly with the introduction paragraph.`;
        ...
        if (error instanceof Error) {
            ...
            if (errorMessage.includes("404") || errorMessage.includes("not found") || errorMessage.includes("not_found")) {
                return { 
                    success: false, 
                    error: "Model not found. Please update your code to use gemini-2.5-flash" 
                };
            }
        }
...
export async function improveContent(content: string, improvementType: ImprovementType = "enhance") {
    try {
        if (!content || content.trim().length === 0)
            throw new Error("Content is required to improve");
